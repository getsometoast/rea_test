...
describe 'moving the robot' do
  ...
  context 'when facing south' do
    ...
    it 'moves the robot forward south' do
      input_file = 'spec/aat/data/move/move_south.txt'
      expect { REATest::Application.new(input_file).run }.to output(/0,0,SOUTH/).to_stdout
    end

    it 'does not move off the table' do
      input_file = 'spec/aat/data/move/move_south_off.txt'
      expect { REATest::Application.new(input_file).run }.to output(/0,0,SOUTH/).to_stdout
    end
  end
  ...
  context 'when facing east' do
    ...
    it 'moves the robot forward east' do
      input_file = 'spec/aat/data/move/move_east.txt'
      expect { REATest::Application.new(input_file).run }.to output(/1,0,EAST/).to_stdout
    end

    it 'does not move off the table' do
      input_file = 'spec/aat/data/move/move_east_off.txt'
  ...
  context 'when facing west' do
    ...
    it 'moves the robot forward west' do
      input_file = 'spec/aat/data/move/move_west.txt'
      expect { REATest::Application.new(input_file).run }.to output(/0,0,WEST/).to_stdout
    end

    it 'does not move off the table' do
      input_file = 'spec/aat/data/move/move_west_off.txt'
      expect { REATest::Application.new(input_file).run }.to output(/0,0,WEST/).to_stdout
    end
  end
end
